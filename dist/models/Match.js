...
MatchSchema.methods.addBall = async function (data) {
    ...
    const bowlerIdxById = this.currentBowlerId
        ? innings.bowlers.findIndex((b) => b.playerId && b.playerId.equals(this.currentBowlerId))
        : -1;
    const bowlerIdx = bowlerIdxById >= 0 ? bowlerIdxById
        : innings.bowlers.findIndex((b) => b.name === this.currentBowlerName);
    if (strikerIdx === -1)
...
MatchSchema.methods.undoLastBall = async function () {
    ...
    if (last.strikerIdBefore)
        this.strikerId = new mongoose_1.default.Types.ObjectId(last.strikerIdBefore);
    if (last.nonStrikerIdBefore)
        this.nonStrikerId = new mongoose_1.default.Types.ObjectId(last.nonStrikerIdBefore);
    ...
    if (last.bowlerPlayerId) {
        this.currentBowlerId = new mongoose_1.default.Types.ObjectId(last.bowlerPlayerId);
    }
