...
const regenerateOverlayUrl = async (req, res) => {
    try {
        ...
        const overlay = await Overlay_1.default.findOneAndUpdate({ _id: req.params.id, createdBy: user._id }, {
    ...
    try {
        ...
        if (authHeader) {
            try {
                ...
                if (payload._id) {
                    userMembership = await checkUserMembership(new mongoose_1.default.Types.ObjectId(payload._id));
                }
            }
            catch (e) {
