...
exports.serveOverlay = exports.getOverlayTemplates = exports.deleteOverlay = exports.updateOverlay = exports.getOverlay = exports.getOverlays = exports.createOverlay = exports.regenerateOverlayUrl = void 0;
...
const regenerateOverlayUrl = async (req, res) => {
    try {
        ...
        const overlay = await Overlay_1.default.findOneAndUpdate({ _id: req.params.id, createdBy: user._id }, { publicId: newPublicId }, { new: true });
        if (!overlay) {
...
const getOverlay = async (req, res) => {
    try {
        const user = req.user;
        const overlay = await Overlay_1.default.findOne({ _id: req.params.id, createdBy: user._id });
        if (!overlay) {
            res.status(404).json({ message: 'Overlay not found' });
            return;
        }
...
const updateOverlay = async (req, res) => {
    try {
        const user = req.user;
        const overlay = await Overlay_1.default.findOneAndUpdate({ _id: req.params.id, createdBy: user._id }, req.body, { new: true });
        if (!overlay) {
            res.status(404).json({ message: 'Overlay not found' });
            return;
        }
...
const deleteOverlay = async (req, res) => {
    try {
        const user = req.user;
        const overlay = await Overlay_1.default.findOneAndDelete({ _id: req.params.id, createdBy: user._id });
        if (!overlay) {
            res.status(404).json({ message: 'Overlay not found' });
            return;
        }
...
const serveOverlay = async (req, res) => {
    try {
        ...
        const possiblePaths = [
            path_1.default.resolve(__dirname, '../../public/overlays'), // Prod structure often dist/../public
            path_1.default.resolve(__dirname, '../../../public/overlays'), // Local dev
            path_1.default.resolve(process.cwd(), 'public/overlays'), // Root execution
            path_1.default.resolve(process.cwd(), 'scorex-frontend/public/overlays') // Monorepo style
        ];
        let templateContent = '';
        let foundLocally = false;
        // 1. Try Local File System
        for (const overlaysDir of possiblePaths) {
            const testPath = path_1.default.join(overlaysDir, templateFile);
            if (fs_1.default.existsSync(testPath)) {
                templateContent = fs_1.default.readFileSync(testPath, 'utf-8');
                foundLocally = true;
                break;
            }
        }
        ...
        if (!foundLocally) {
            ...
            try {
                const templateResponse = await axios_1.default.get(templateUrl, {
                    timeout: 5000,
                    responseType: 'text'
                });
