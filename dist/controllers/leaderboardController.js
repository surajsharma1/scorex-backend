...
const getGlobalLeaderboard = async (req, res, next) => {
    try {
        ...
        if (timeframe !== 'all') {
            ...
            if (dates[timeframe])
                query.lastMatchDate = { $gte: dates[timeframe] };
        }
        ...
        if (type === 'player') {
            ...
            total = await Player_1.default.countDocuments(query);
        }
        else if (type === 'team') {
            ...
            total = await Team_1.default.countDocuments(query);
        }
        else {
...
const getTournamentLeaderboard = async (req, res, next) => {
    try {
        ...
        let data = [];
        if (type === 'player') {
            const teams = await Team_1.default.find({ tournaments: tournamentId }).populate('players');
            const playerIds = teams.flatMap((t) => (t.players || []).map((p) => p._id));
            const matches = await Match_1.default.find({ tournamentId, status: 'completed' });
            ...
            const players = await Player_1.default.find({ _id: { $in: playerIds } }).select('name profilePicture role').lean();
            data = players.map(p => {
                const s = playerStats.get(p._id.toString()) || {};
                const points = (s.runs || 0) + (s.fours || 0) + (s.sixes || 0) * 2 + (s.wickets || 0) * 10 + (s.catches || 0) * 5;
                return { _id: p._id, name: p.name, photo: p.profilePicture, role: p.role, points, ...s };
            });
...
const getMatchLeaderboard = async (req, res, next) => {
    try {
        const match = await Match_1.default.findById(req.params.matchId).populate('team1', 'name shortName').populate('team2', 'name shortName');
        if (!match)
...
const getOrangeCap = async (req, res, next) => {
    try {
        ...
        if (tournamentId) {
            ...
            query._id = { $in: teams.flatMap((t) => t.players || []) };
        }
        const players = await Player_1.default.find(query).sort({ 'battingStats.totalRuns': -1 }).limit(10).select('name profilePicture battingStats').lean();
...
const getPurpleCap = async (req, res, next) => {
    try {
        ...
        const players = await Player_1.default.find(query).sort({ 'bowlingStats.totalWickets': -1 }).limit(10).select('name profilePicture bowlingStats').lean();
