...
export interface IInnings {
  ...
  score: number;
  wickets: number;
  overs: number;        // e.g. 4.3 means 4 complete overs + 3 balls
  ...
  // Ball-by-ball history for undo
  ballHistory: Array<{
    over: number;
    ball: number;
    runs: number;
    extras: string;     // 'wide'|'nb'|'bye'|'lb'|''
    wicket: boolean;
    outType?: string;
    batsmanName: string;
    bowlerName: string;
...
export interface IMatch extends Document {
  ...
  // Active players (names for fast access)
  strikerName: string;
  nonStrikerName: string;
  currentBowlerName: string;
  // Summary fields (denormalized for fast read)
  team1Score: number;
  team1Wickets: number;
  team1Overs: number;
  team2Score: number;
...
// HELPERS
// ============================================
function formatOvers(completedOvers: number, ballsInOver: number): string {
...
function calcRequiredRunRate(required: number, remainingOvers: number, remainingBalls: number): number {
...
MatchSchema.methods.addBall = async function(data: AddBallData): Promise<ScoreUpdateResult> {
  ...
  if (isWicket) {
    ...
    const outBatsman = data.outBatsmanName
      ? innings.batsmen.find((b: IBatsman) => b.name === data.outBatsmanName && !b.isOut)
      : striker;
...
MatchSchema.methods.endInnings = async function(): Promise<void> {
  ...
  if (this.currentInnings === 1) {
    ...
    this.innings.push({
      teamId: new mongoose.Types.ObjectId(secondBattingTeamId),
...
MatchSchema.methods.getOverSummary = function(): string {
  ...
  return thisOverBalls.map((b: any) => {
    if (b.wicket) return 'W';
    if (b.extras === 'wide') return 'Wd';
    if (b.extras === 'nb') return 'Nb';
    if (b.extras === 'bye') return `B${b.runs}`;
    if (b.extras === 'lb') return `Lb${b.runs}`;
