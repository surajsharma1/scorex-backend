...
export interface IBatsman {
  ...
  isStriker: boolean;
  outType?: string;
  outTo?: string;       // bowler name
  outFielder?: string;  // fielder for catch/run-out
  enteredAt?: number;   // ball number when batsman came in
}
...
export interface IMatch extends Document {
  ...
  startMatch(data: StartMatchData): Promise<void>;
  endInnings(): Promise<void>;
  endMatch(winnerId?: string, winnerName?: string, resultSummary?: string): Promise<void>;
  undoLastBall(): Promise<void>;
  selectPlayers(data: SelectPlayersData): Promise<void>;
  getOverSummary(): string;
  _updateSummary(innings: IInnings): void;
}

export interface AddBallData {
  runs?: number; wide?: boolean; noBall?: boolean; bye?: number; legBye?: number;
...
export interface StartMatchData {
  tossWinnerId: string; tossWinnerName: string; tossDecision: TossDecision;
...
export interface ScoreUpdateResult {
  score: number; wickets: number; overs: string; runRate: number;
  requiredRuns?: number; requiredRunRate?: number; targetScore?: number;
  ballDescription: string; overChanged: boolean; inningsEnded: boolean;
  matchEnded: boolean; needPlayerSelection: boolean;
}

// ============================================
// SCHEMA
// ============================================
const BatsmanSchema = new Schema<IBatsman>({ 
...
const InningsSchema = new Schema<IInnings>({ 
  ...
  fallOfWickets: [Schema.Types.Mixed], 
  ballHistory: [Schema.Types.Mixed] 
}, { _id: false });
...
MatchSchema.methods.startMatch = async function(data: StartMatchData): Promise<void> {
  this.tossWinner = new mongoose.Types.ObjectId(data.tossWinnerId);
  ...
  if (this.format !== 'Custom') this.maxOvers = oversMap[this.format] || 20;

  this.innings = [{
    teamId: new mongoose.Types.ObjectId(data.battingTeamId), teamName: data.battingTeamName, status: 'in_progress',
    score: 0, wickets: 0, overs: 0, balls: 0, runRate: 0,
    extras: { wides: 0, noBalls: 0, byes: 0, legByes: 0, total: 0 },
    batsmen: [
      { name: data.striker, runs: 0, balls: 0, fours: 0, sixes: 0, strikeRate: 0, isOut: false, isStriker: true, enteredAt: 0 },
      { name: data.nonStriker, runs: 0, balls: 0, fours: 0, sixes: 0, strikeRate: 0, isOut: false, isStriker: false, enteredAt: 0 }
    ],
...
MatchSchema.methods.addBall = async function(data: AddBallData): Promise<ScoreUpdateResult> {
  ...
  // --- INTERCEPT: RETIREMENTS & PENALTIES (No dot balls added) ---
  if (data.retired || data.penalty) {
    ...
    await this.save();
    return {
      score: innings.score, wickets: innings.wickets, overs: formatOvers(innings.overs, innings.balls % 6), runRate: innings.runRate,
      requiredRuns: innings.requiredRuns, requiredRunRate: innings.requiredRunRate, targetScore: innings.targetScore,
      ballDescription: data.retired ? 'Retired' : `+${data.penalty} Penalty`,
      overChanged: false, inningsEnded: innings.wickets >= 10, matchEnded: false, needPlayerSelection: !!data.retired
    };
  }

  const runs = data.runs || 0; const isWide = data.wide || false; const isNoBall = data.noBall || false;
  const byeRuns = data.bye || 0; const legByeRuns = data.legBye || 0; const isWicket = data.wicket || false;
  ...
  if (isWide) {
    ...
    innings.extras.wides += 1; innings.extras.total += extrasRuns; innings.score += extrasRuns;
    if (bowler) { bowler.runs += extrasRuns; bowler.wides += 1; }
    ballDesc = `Wide${runs > 0 ? `+${runs}` : ''}`; isLegalDelivery = false;
  } else if (isNoBall) {
    extrasRuns = 1; innings.extras.noBalls += 1; innings.extras.total += 1 + runs + byeRuns + legByeRuns; innings.score += 1 + runs + byeRuns + legByeRuns;
    if (bowler) { bowler.runs += 1 + runs; bowler.noBalls += 1; }
    if (runs > 0) { striker.runs += runs; if (runs === 4) striker.fours += 1; if (runs === 6) striker.sixes += 1; }
    striker.strikeRate = striker.balls > 0 ? parseFloat(((striker.runs / striker.balls) * 100).toFixed(1)) : 0;
    ballDesc = `NB${runs > 0 ? `+${runs}` : ''}`; isLegalDelivery = false; 
  } else if (byeRuns > 0) {
    extrasRuns = byeRuns; innings.extras.byes += byeRuns; innings.extras.total += byeRuns; innings.score += byeRuns;
    striker.balls += 1; if (bowler) bowler.balls += 1; ballDesc = `B${byeRuns}`;
  } else if (legByeRuns > 0) {
    extrasRuns = legByeRuns; innings.extras.legByes += legByeRuns; innings.extras.total += legByeRuns; innings.score += legByeRuns;
    striker.balls += 1; if (bowler) bowler.balls += 1; ballDesc = `LB${legByeRuns}`;
  } else {
    innings.score += runs; striker.runs += runs; striker.balls += 1;
    if (runs === 4) striker.fours += 1; if (runs === 6) striker.sixes += 1;
  ...
  if (isWicket) {
    ...
    if (outBatsman) {
      outBatsman.isOut = true; outBatsman.outType = data.outType || 'bowled'; outBatsman.outTo = this.currentBowlerName; outBatsman.outFielder = data.outFielder; outBatsman.isStriker = false;
      innings.fallOfWickets.push({ wicket: innings.wickets, score: innings.score, overs: formatOvers(innings.overs, innings.balls % 6), batsman: outBatsman.name, bowler: this.currentBowlerName });
  ...
  if (runsForRotation % 2 === 1) {
    // Swap Names completely (Even if one of them is Out)
    ...
    innings.batsmen.forEach((b: IBatsman) => {
      ...
      else if (b.name === this.nonStrikerName) b.isStriker = false;
    });
  }
  ...
  return {
    score: innings.score, wickets: innings.wickets, overs: formatOvers(innings.overs, innings.balls % 6), runRate: innings.runRate,
    requiredRuns: innings.requiredRuns, requiredRunRate: innings.requiredRunRate, targetScore: innings.targetScore,
    ballDescription: ballDesc, overChanged, inningsEnded, matchEnded, needPlayerSelection: needPlayerSelection && !matchEnded
  };
};

// ============================================
// _updateSummary METHOD
// ============================================
...
MatchSchema.methods.undoLastBall = async function(): Promise<void> {
  ...
  innings.score = last.totalBefore; innings.wickets = last.wicketsBefore;
  ...
  if (batsman && !last.extras.includes('wide')) {
    if (batsman.balls > 0) batsman.balls -= 1; batsman.runs -= last.runs;
    if (last.runs === 4 && batsman.fours > 0) batsman.fours -= 1; if (last.runs === 6 && batsman.sixes > 0) batsman.sixes -= 1;
  ...
  if (bowler) {
    if (!last.extras.includes('wide') && !last.extras.includes('nb')) { if (bowler.balls > 0) bowler.balls -= 1; bowler.overs = Math.floor(bowler.balls / 6); }
    if (last.wicket && bowler.wickets > 0) bowler.wickets -= 1;
    bowler.runs -= last.runs; bowler.economy = bowler.overs > 0 ? parseFloat((bowler.runs / bowler.overs).toFixed(2)) : 0;
  }
  ...
  innings.runRate = calcRunRate(innings.score, innings.overs, innings.balls % 6);
  this._updateSummary(innings); await this.save();
};
...
MatchSchema.methods.selectPlayers = async function(data: SelectPlayersData): Promise<void> {
  ...
  if (data.striker) {
    ...
    } else { innings.batsmen.forEach((b: IBatsman) => { b.isStriker = false; }); existing.isStriker = true; }
  }
